...
export type ProjectInput = {
  ...
  country: string;
  capacity_kw: number;
  description: string; // we'll store off-chain / IPFS later → becomes meta_cid
};

type Props = {
  devAddr: string;                // connected wallet (for display only right now)
...
export default function ProjectForm({ devAddr, onSubmit }: Props) {
  ...
  function update<K extends keyof ProjectInput>(k: K, v: ProjectInput[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.name.trim()) return enqueueSnackbar("Please enter a project name.", { variant: "warning" });
    if (!form.country.trim()) return enqueueSnackbar("Please select a country.", { variant: "warning" });
    if (!form.capacity_kw || form.capacity_kw <= 0) return enqueueSnackbar("Capacity must be > 0.", { variant: "warning" });
